...
-- 익명의 반복 프로시저 실습
-- 사원(emp)테이블로부터 사원번호 2002인 사원의 사원번호, 사원명, 직급, 주소, 입사일을 출력하시오.
...
EXEC ins_emp(2011,'김다연','사원', 2350000);
EXEC ins_emp(2012,'히카루','사원', 2320000);
EXEC ins_emp(2013,'서영은','사원', 2320000);    
    
-- CREATE OR REPLACE PROCEDURE ins_emp(ins_eno in emp.eno%TYPE, ins_ename in emp.ename%TYPE, ins_pno in emp.pno%TYPE, 
-- ins_pos in emp.pno%TYPE, ins_pcode in emp.pcode%TYPE,  ins_addr in emp.addr%TYPE, vtel IN emp.tel%TYPE,
-- ins_salary IN emp.salary%TYPE, ins_bonus IN emp.bonus%TYPE, ins_regdate IN emp.regdate%TYPE, ins_gender IN emp.gender%TYPE,
-- ins_superior IN emp.superior%TYPE)
-- IS
-- BEGIN 
...
SELECT circle_fnc(1) as "면적" from dual;
...
END;
/
SELECT vol_fnc(20,5,8) as "부피" from dual;
-- 반환값이 ... dual
...
----------------------------
-- 오답  CREATE OR REPLACE FUNCTION workdays_fnc(a_day in varchar b_day in varchar)
--RETURN NUMBER
--IS 
--    workday NUMBER;
--BEGIN
...
-- 3) WHILE LOOP 문을 활용하여 번호를 증가시키고 자동 채우면서 10개의 레코드를 추가될 수 있도록 반복할 것.
